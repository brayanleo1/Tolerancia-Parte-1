...
def request0():
    ...
    product = requests.get('http://localhost:5000/product', headers=headers, data=product_data)
    ...
    exchange = requests.get('http://localhost:5000/exchange', headers=headers)
    ...
    sell = requests.post('http://localhost:5000/sell', headers=headers, data=sell_data)
    ...
    bonus_data = {
        "user_id": user_id,
        "bonus_value": round(product.json()["value"])
    }

    # Request to the fidelity service
    bonus = requests.post('http://localhost:5000/bonus', headers=headers, data=bonus_data)
